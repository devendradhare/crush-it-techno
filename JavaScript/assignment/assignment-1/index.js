...
Product_data.forEach((el) => {
  ...
  smallImgDiv.className = "smallImgDiv";
  for (let i = 0; i < el.images.length; i++) {
    let smallImg = document.createElement("img");
    smallImg.src = el.images[i];
    smallImg.addEventListener("click", changeToThisImage);
    smallImgDiv.append(smallImg);
  }
  ...
  let dP = (el.price * el.discountPercentage) / 100;
  discountPrice.innerText = (el.price - dP).toFixed(2);
  let discountPercentage = document.createElement("span");
  discountPercentage.innerText = el.discountPercentage + "%";
  priceDiv.append(price, discountPrice, discountPercentage);

  let rating = document.createElement("span");
  let stars = "";
  let i = 1;
  for (i = 1; i <= 5; i++) {
    if (i < el.rating) stars = stars + "🌕";
    else if (i - el.rating < 1) stars = stars + "🌗";
    else stars = stars + "🌑";
  }
  rating.innerText = stars + " " + el.rating;
...
function changeToThisImage() {
  console.log(event.target.parentNode.parentNode.querySelector(".images"));
  console.log(event.target.src);
  event.target.parentNode.parentNode.querySelector(".images").src =
    event.target.src;
}
